...
**⚠️ 不要把這裡想成一般的「髒 Excel 清理」。** 機台 log 匯進 Excel 之後整包擠在單一欄位，
**沒有合併儲存格、沒有多層表頭、沒有多個工作表**。早期版本的本節曾這樣描述，那是錯的。
...
- **輸入格式：真的需要先匯進 Excel 嗎？** log 既然是純文字，後端直接吃 `.txt` / `.log` / `.csv` 可以讓使用者少一個步驟。待確認同事拿得到原始檔還是只拿得到 Excel
- **Univer 的成本效益要重新評估** —— 選它時以為要處理排版過的髒 Excel；實際是純文字 dump，需要的只是「preview 格線 + 點值篩選」。完整的 Univer（React + canvas + 公式引擎）對斷網 image 是不小的負擔，輕量 data grid 可能就夠
